...
{
	[Authorize]
    [Route("api/thema/more")]
    public class ThemaMoreController : Controller
    {

        private readonly IThemaRepository _rep;
        private readonly IThemaApplicationService _app;
		private readonly ILogger _logger;
		private readonly EnviromentInfo _env;
		private readonly CurrentUser _user;

        public ThemaMoreController(IThemaRepository rep, IThemaApplicationService app, ILoggerFactory logger, EnviromentInfo env,CurrentUser user)
        {
            this._rep = rep;
            this._app = app;
			this._logger = logger.CreateLogger<ThemaMoreController>();
			this._env = env;
			this._user = user;
        }
        ...
        public async Task<IActionResult> Get([FromQuery]ThemaFilter filters)
        {
            ...
            try
            {
				...
				if (!this._user.GetClaims().GetTools().VerifyClaimsCanRead("Thema"))
                {
				...
				if (filters.FilterBehavior == FilterBehavior.Export)
                {
                    ...
                    var file = export.ExportFile(this.Response, searchResult, "Thema", this._env.RootPath);
        ...
        [HttpPost]
        public async Task<IActionResult> Post([FromBody]IEnumerable<ThemaDtoSpecialized> dtos)
        {
            ...
            {
                return result.ReturnCustomException(ex,"Score.Platform.Account - Thema", dtos, new ErrorMapCustom());
            }
		...
		[Authorize(Policy = "CanWrite")]
		[HttpPut]
        public async Task<IActionResult> Put([FromBody]IEnumerable<ThemaDtoSpecialized> dtos)
        {
            var result = new HttpResult<ThemaDto>(this._logger);
            try
            {

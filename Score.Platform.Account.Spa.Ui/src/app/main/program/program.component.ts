...
export class ProgramComponent extends ComponentBase implements OnInit, OnDestroy {
    ...
    constructor(private programService: ProgramService, private router: Router, private ref: ChangeDetectorRef) {
    ...
    ngOnInit() {
        ...
        this.programService.OnHide(this.saveModal, this.editModal, () => { this.hideComponents() });

        this.programService.get(this.vm.modelFilter).subscribe((result) => {
    ...
    public onFilter(modelFilter: any) {
        ...
        this.programService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
    ...
    public onExport() {
        this.programService.export().subscribe((result) => {
    ...
    public onCreate() {
        ...
        this.navigateStrategy(this.vm, this.saveModal, this.router, "/program/create");
    }
    ...
    public onEdit(model: any) {
        ...
        if (model)  {
            newModel = { id: model.programId }
        }

        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/program/edit/" + newModel.id);
        }
        else {
            this.programService.get(newModel).subscribe((result) => {
    ...
    public onSave(model: any) {
        ...
        this.programService.save(model).subscribe((result) => {
            ...
            this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                return  model.programId != result.data.programId;
            });
    ...
    public onDetails(model: any) {
        ...
        if (!this.vm.navigationModal) {
            this.navigateStrategy(this.vm, this.editModal, this.router, "/program/details/" + newModel.id);
        }
        else {
            this.programService.get(newModel).subscribe((result) => {
    ...
    public onPrint(model: any) {
        this.router.navigate(['/program/print', model.programId]);
    }
    ...
    public onDeleteConfimation(model: any) {
        ...
        var conf = GlobalService.operationExecutedParameters(
            ...
            () => {
                ...
                this.programService.delete(newModel).subscribe((result) => {
                    this.vm.filterResult = this.vm.filterResult.filter(function (model) {
                        return  model.programId != result.data.programId;
                    });
                    this.vm.summary.total = this.vm.filterResult.length
    ...
    public onPageChanged(pageConfig: any) {
        ...
        let modelFilter = this.programService.pagingConfig(this.vm.modelFilter, pageConfig);
        this.programService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
    ...
    public onOrderBy(order: any) {
        ...
        let modelFilter = this.programService.orderByConfig(this.vm.modelFilter, order);
        this.programService.get(modelFilter).subscribe((result) => {
            this.vm.filterResult = result.dataList;
            this.vm.summary = result.summary;
        });
